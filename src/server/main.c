...
static const char *const kBotNamePrefixes[] = {
    "Mycelium", "Sporecap", "Hyphae", "Spore",
    "Fungal",   "Myco",     "Shroom", "Mold",
};
...
typedef struct ServerSession {
  ...
  uint32_t
      focused_entity_id; /* entity_id of piece being controlled; 0 = primary */
...
static void CopyConfigString(char *destination, size_t destination_size,
                             const char *source) {
...
static void PrintUsage(const char *program_name) {
  ...
  printf(
      "  --match-duration SECONDS  Match duration in seconds, default %.0f\n",
      SHROOM_MATCH_DURATION_SECONDS);
  printf("  --smoke-test      Start, initialize subsystems, then shut down "
         "cleanly\n");
  printf("  --benchmark       Run deterministic server simulation benchmark "
         "and exit\n");
...
static bool ResolveBindAddress(const char *bind_value,
                               enet_uint32 *bind_address) {
  ...
  if (sscanf(bind_value, "%u.%u.%u.%u%c", &octets[0], &octets[1], &octets[2],
             &octets[3], &trailing) != 4) {
    return false;
  }
  ...
  if ((octets[0] == 0u) && (octets[1] == 0u) && (octets[2] == 0u) &&
      (octets[3] == 0u)) {
    *bind_address = ENET_HOST_ANY;
    return true;
  }

  *bind_address = ENET_HOST_TO_NET_32((octets[0] << 24u) | (octets[1] << 16u) |
...
static bool LoadServerConfig(ServerConfig *config, int argc, char **argv) {
  ...
  *config =
      (ServerConfig){.port = (uint16_t)SHROOM_SERVER_PORT,
  ...
  CopyConfigString(config->database_path, sizeof(config->database_path),
                   "shroomio.db");
  ...
  if ((env_port != NULL) && (env_port[0] != '\0') &&
      !ParsePort(env_port, &config->port)) {
  ...
  while (i < argc) {
    if (strcmp(argv[i], "--help") == 0) {
      ...
      ++i;
      if (!ParseUint32(argv[i], &config->benchmark_ticks) ||
          (config->benchmark_ticks == 0u)) {
...
static void ServerProfileMaybeLog(uint64_t now_ms) {
  ...
  LOG_INFO("profile,server,tick_avg_ms=%.3f,tick_peak_ms=%.3f,enet_avg_ms=%.3f,"
           "enet_peak_ms=%.3f,simulation_avg_ms=%.3f,simulation_peak_ms=%.3f,"
           "broadcast_avg_ms=%.3f,broadcast_peak_ms=%.3f",
           ShroomProfileAverageMs(&g_server_profile.tick),
           g_server_profile.tick.peak_ms,
           ShroomProfileAverageMs(&g_server_profile.enet_events),
           g_server_profile.enet_events.peak_ms,
           ShroomProfileAverageMs(&g_server_profile.simulation),
           g_server_profile.simulation.peak_ms,
           ShroomProfileAverageMs(&g_server_profile.broadcast),
           g_server_profile.broadcast.peak_ms);
...
static int RunServerBenchmark(const ServerConfig *config) {
  ...
  for (uint32_t player = 0u; player < config->benchmark_bots; ++player) {
    ShroomPlayerState *spawned =
        ShroomWorldSpawnPlayer(&world, player + 1u, true);
    if (spawned != NULL) {
      spawned->input_direction =
          NormalizeInput((ShroomVec2){(player % 3u) == 0u ? 1.0f : -0.35f,
                                      (player % 5u) == 0u ? 0.65f : -0.2f});
    }
  }
  ...
  for (uint32_t tick = 0u; tick < config->benchmark_ticks; ++tick) {
    ...
    const double sim_ms =
        ShroomProfileNanosToMs(GetTimeNanos() - sim_start_nanos);
    ...
    if ((snapshot_interval_ticks > 0u) &&
        ((world.tick % snapshot_interval_ticks) == 0u)) {
      ...
      estimated_snapshot_bytes +=
          ...
          (uint64_t)(offsetof(ShroomSnapshotPacket, players) +
                     ((size_t)world.player_count *
                      sizeof(ShroomSnapshotPlayerState)));
    }
    if ((spore_interval_ticks > 0u) &&
        ((world.tick % spore_interval_ticks) == 0u)) {
      estimated_packet_count += (uint64_t)config->benchmark_bots * 2ull;
      estimated_snapshot_bytes +=
          (uint64_t)config->benchmark_bots *
          (uint64_t)((sizeof(ShroomPacketHeader) + sizeof(uint64_t) +
                      (2u * sizeof(uint16_t)) +
                      ((size_t)world.spore_count *
                       sizeof(ShroomSnapshotSporeState))) +
                     sizeof(ShroomPowerupStatePacket));
    }
  }
  elapsed_nanos = GetTimeNanos() - started_nanos;

  printf("scenario,players,ticks,elapsed_ms,avg_tick_ms,worst_tick_ms,"
         "estimated_packets,"
         "estimated_bytes,rtt_ms,cpu_time_ms,memory_kb\n");
  printf("server_bots,%u,%u,%.3f,%.3f,%.3f,%llu,%llu,0,%.3f,0\n",
         config->benchmark_bots, config->benchmark_ticks,
         ShroomProfileNanosToMs(elapsed_nanos),
         sim_sum_ms / (double)config->benchmark_ticks, sim_peak_ms,
         (unsigned long long)estimated_packet_count,
         (unsigned long long)estimated_snapshot_bytes,
         ShroomProfileNanosToMs(elapsed_nanos));
  return 0;
}
...
static void SendWelcome(ENetPeer *peer) {
  const ShroomWelcomePacket packet = {
      .header = {SHROOM_PACKET_WELCOME, SHROOM_ENET_CHANNEL_CONTROL,
...
static void SendMushroomSpeciesCatalog(ENetPeer *peer, sqlite3 *db) {
  static const char *const sql =
      "SELECT species_id, name, description, pattern_id, rarity_tier, "
      "cap_color_rgba "
      "FROM mushroom_species WHERE unlocked_by_default = 1 ORDER BY sort_order "
      "ASC";
  ...
  if (sqlite3_prepare_v2(db, sql, -1, &stmt, NULL) != SQLITE_OK) {
    LOG_ERROR("failed to prepare mushroom species catalog query: %s",
              sqlite3_errmsg(db));
    return;
  }

  while ((count < SHROOM_MAX_MUSHROOM_SPECIES) &&
  ...
  enet_peer_send(peer, SHROOM_ENET_CHANNEL_CONTROL,
                 CreateProtocolPacket(&packet, packet_size,
                                      SHROOM_PACKET_MUSHROOM_SPECIES_CATALOG));
}
...
static uint16_t GetLobbyBotTarget(uint16_t real_player_count) {
  ...
  const uint32_t max_bots_for_capacity =
      real_player_count >= SHROOM_MAX_PLAYERS
          ? 0u
...
static void AssignBotName(ShroomPlayerState *bot, uint32_t bot_number) {
  ...
  snprintf(bot->name, sizeof(bot->name), "%s-%u",
           kBotNamePrefixes[(bot_number - 1u) % BOT_NAME_PREFIX_COUNT],
           bot_number);
}
...
static void SendLobbyList(ENetPeer *peer, ShroomLobby *lobbies,
                          const ENetHost *host) {
  ...
  for (i = 0; i < SHROOM_MAX_LOBBIES && count < SHROOM_MAX_LOBBIES; ++i) {
    ...
    snprintf(packet.lobbies[count].name, sizeof(packet.lobbies[count].name),
             "%s", lobby->name);
    ++count;
  }
...
static void BroadcastLobbyRoster(ENetHost *host, uint32_t lobby_id) {
  ...
  for (index = 0;
       index < host->peerCount && packet.player_count < SHROOM_MAX_PLAYERS;
       ++index) {
    ...
    if ((peer->state != ENET_PEER_STATE_CONNECTED) || (session == NULL) ||
        !session->active || (session->lobby_id != lobby_id)) {
      continue;
    }
  ...
  ShroomPacketHeaderInit(&packet.header, SHROOM_PACKET_LOBBY_ROSTER,
                         (uint16_t)packet_size);
...
static void SendLobbyCreated(ENetPeer *peer, const ShroomLobby *lobby) {
  ...
  ShroomPacketHeaderInit(&packet.header, SHROOM_PACKET_LOBBY_CREATED,
                         sizeof(packet));
  packet.lobby_id = lobby->lobby_id;
  snprintf(packet.name, sizeof(packet.name), "%s", lobby->name);
  enet_peer_send(peer, SHROOM_ENET_CHANNEL_CONTROL,
                 CreateProtocolPacket(&packet, sizeof(packet),
                                      SHROOM_PACKET_LOBBY_CREATED));
}

static void SendPong(ENetPeer *peer, uint32_t nonce) {
  const ShroomPongPacket packet = {
      .header = {SHROOM_PACKET_PONG, SHROOM_ENET_CHANNEL_CONTROL,
...
static void SendAuthResponse(ENetPeer *peer, ShroomAuthResult result,
                             uint32_t player_id, const char *token,
                             const char *message) {
  ShroomAuthResponsePacket packet = {0};
  ShroomPacketHeaderInit(&packet.header, SHROOM_PACKET_AUTH_RESPONSE,
                         sizeof(packet));
...
static void SendSnapshot(ENetPeer *peer, const ServerSession *session,
  ...
  for (index = 0; (index < world->player_count) &&
    ...
    packet.players[player_count++] = (ShroomSnapshotPlayerState){
        ...
        .effect_flags = (uint16_t)((player->speed_powerup_timer > 0.0f
                                        ? SHROOM_POWERUP_EFFECT_SPEED
                                        : 0u) |
                                   (player->shield_powerup_timer > 0.0f
                                        ? SHROOM_POWERUP_EFFECT_SHIELD
                                        : 0u) |
                                   (player->magnet_powerup_timer > 0.0f
                                        ? SHROOM_POWERUP_EFFECT_MAGNET
                                        : 0u) |
                                   (player->decay_immune_powerup_timer > 0.0f
                                        ? SHROOM_POWERUP_EFFECT_DECAY_IMMUNE
                                        : 0u)),
    };
    snprintf(packet.players[player_count - 1].name,
             sizeof(packet.players[player_count - 1].name), "%s", player->name);
  }
...

  {
    ...
    enet_peer_send(
        peer, SHROOM_ENET_CHANNEL_SNAPSHOT,
...
static void SendSporeState(ENetPeer *peer, const ShroomWorldState *world) {
  ...
  if (spore_count == 0u) {
    ...
    ShroomSporeStatePacket *packet =
        (ShroomSporeStatePacket *)enet_packet->data;
...
static void LogPeerLatency(ENetPeer *peer, ServerSession *session,
                           uint64_t now_ms) {
  uint32_t rtt_ms;
  ...
  if (rtt_ms >= SHROOM_LATENCY_UNPLAYABLE_MS) {
    LOG_WARN("high latency peer: slot=%u player_id=%u rtt=%ums",
             (unsigned)peer->incomingPeerID, session->player_id, rtt_ms);
  } else {
    LOG_INFO("latency warning peer: slot=%u player_id=%u rtt=%ums",
             (unsigned)peer->incomingPeerID, session->player_id, rtt_ms);
  }
}
...
static void HandleHelloPacket(ENetPeer *peer, ServerSession *session,
  ...
  const ShroomHelloPacket *packet =
      (const ShroomHelloPacket *)enet_packet->data;
...
static void AdjustSessionAiControl(ServerSession *session,
                                   ShroomWorldState *world) {
  size_t i;
  bool focused_alive = false;

  if ((session == NULL) || !session->active || (session->player == NULL) ||
      (world == NULL)) {
    return;
  }
  ...
  if (!focused_alive) {
    /* Focused piece gone: reset to primary and clear ai_controlled on all
     * pieces. */
...
static void HandleInputPacket(ServerSession *session,
                              const ENetPacket *enet_packet,
                              ShroomWorldState *world) {
  const ShroomInputPacket *packet =
      (const ShroomInputPacket *)enet_packet->data;
  ShroomPlayerState *target_piece;
  size_t i;

  if ((session == NULL) || !session->active || session->spectating ||
      (session->player == NULL)) {
    return;
  }
  if (enet_packet->dataLength < sizeof(*packet)) {
  ...
  ShroomPlayerSetInput(
      target_piece,
      NormalizeInput((ShroomVec2){packet->direction_x, packet->direction_y}));

  /* Human split pieces without focus keep their last direction; bot pieces keep
   * AI. */
  if (world != NULL) {
    for (i = 0; i < world->player_count; ++i) {
      ShroomPlayerState *p = &world->players[i];
      if (p->alive && (p->player_id == session->player_id)) {
        p->ai_controlled =
            p->is_bot && (p->entity_id != session->focused_entity_id);
      }
    }
  }
  ...
  if (packet->split_requested && (world != NULL)) {
    ShroomWorldSplitPlayerToward(
        world, target_piece,
        NormalizeInput((ShroomVec2){packet->split_direction_x,
                                    packet->split_direction_y}));
  }
...
static void HandleAuthRequestPacket(ENetPeer *peer, ServerSession *session,
                                    ...
                                    const ENetPacket *enet_packet) {
  const ShroomAuthRequestPacket *packet =
      (const ShroomAuthRequestPacket *)enet_packet->data;
  ...
  if (session->authenticated) {
    SendAuthResponse(peer, SHROOM_AUTH_INVALID_INPUT, 0, NULL,
  ...
  if (method == SHROOM_AUTH_PASSWORD) {
    if (is_register) {
      ShroomAuthUser user;
      ShroomAuthResult result = ShroomAuthRegister(auth_ctx, packet->username,
                                                   packet->password, &user);
      if (result == SHROOM_AUTH_SUCCESS) {
        ShroomAuthToken token;
        result = ShroomAuthLogin(auth_ctx, packet->username, packet->password,
                                 &token);
        if (result == SHROOM_AUTH_SUCCESS) {
          session->authenticated = true;
          session->user_id = token.user_id;
          strncpy(session->auth_token, token.token,
                  sizeof(session->auth_token) - 1);
          session->auth_token[sizeof(session->auth_token) - 1] = '\0';
          SendAuthResponse(peer, result, user.player_id, token.token,
                           "Registration successful");
          LOG_INFO("Auth: User '%s' registered and logged in",
                   packet->username);
        } else {
          SendAuthResponse(peer, result, 0, NULL,
                           "Login after registration failed");
        }
      } else {
        const char *msg = (result == SHROOM_AUTH_USERNAME_TAKEN)
    ...
    ShroomAuthToken token;
    ShroomAuthResult result =
        ShroomAuthLoginAnonymous(auth_ctx, packet->username, &token);
    if (result == SHROOM_AUTH_SUCCESS) {
      ShroomAuthUser user;
      ShroomAuthValidateToken(auth_ctx, token.token, &user);
      session->authenticated = true;
...
static void HandleChatPacket(ENetHost *host, ServerSession *session,
                             const ENetPacket *enet_packet, uint64_t now_ms) {
  ShroomChatPacket broadcast = {0};
  size_t index;
  size_t msg_len;

  if ((host == NULL) || (session == NULL) || !session->active ||
      (session->player == NULL) || (enet_packet == NULL) ||
      (enet_packet->dataLength < sizeof(ShroomChatPacket))) {
    return;
  }

  /* Rate limit: max SHROOM_CHAT_RATE_LIMIT_COUNT messages per window. */
  if (now_ms - session->chat_window_start_ms >=
      SHROOM_CHAT_RATE_LIMIT_WINDOW_MS) {
  ...
  ShroomPacketHeaderInit(&broadcast.header, SHROOM_PACKET_CHAT,
                         sizeof(broadcast));
  broadcast.sender_id = session->player_id;
  snprintf(broadcast.sender_name, sizeof(broadcast.sender_name), "%s",
           session->player->name[0] != '\0' ? session->player->name
                                            : "Unknown");
  ...
  memcpy(broadcast.message,
         ((const ShroomChatPacket *)enet_packet->data)->message, msg_len);
  ...
  LOG_INFO("chat player_id=%u name=%.31s msg=%s", session->player_id,
           broadcast.sender_name, broadcast.message);
  ...
  for (index = 0; index < host->peerCount; ++index) {
    ...
    if ((peer_session == NULL) ||
        (peer_session->lobby_id != session->lobby_id)) {
      continue;
    }
    out =
        CreateProtocolPacket(&broadcast, sizeof(broadcast), SHROOM_PACKET_CHAT);
    if (out != NULL) {
...
static void HandleReadyStatePacket(ENetHost *host, ServerSession *session,
  ...
  if ((host == NULL) || (session == NULL) || !session->active ||
      (session->player == NULL) || (enet_packet == NULL) ||
      (enet_packet->dataLength < sizeof(ShroomReadyStatePacket))) {
    return;
  }
  ...
  LOG_INFO("ready state player_id=%u ready=%d", session->player_id,
           session->is_ready);
...
static void HandleEnterMatchPacket(ENetHost *host, ServerSession *session,
  ...
  if ((session->lobby_id == 0u) || (packet->lobby_id != session->lobby_id) ||
      (!session->spectating &&
       ((session->player == NULL) || !session->is_ready))) {
    return;
  }
  if (session->entered_match) {
    return;
  ...
  if (session->player != NULL) {
    session->player->input_direction = (ShroomVec2){0};
    session->player->spawn_protection_timer =
        SHROOM_PLAYER_SPAWN_PROTECTION_SECONDS;
  }
  LOG_INFO("match entry: player_id=%u lobby_id=%u spectating=%d",
           session->player_id, session->lobby_id, (int)session->spectating);
  BroadcastLobbyRoster(host, session->lobby_id);
}
...
static void AdjustLobbyBots(ShroomLobby *lobby, const ENetHost *host,
...
static ShroomLobby *CreateLobby(ShroomLobby *lobbies, uint32_t lobby_id,
                                const char *name, bool is_dynamic,
  ...
  for (i = 0; i < SHROOM_MAX_LOBBIES; ++i) {
    if (!lobbies[i].active) {
      ...
      *lobby = (ShroomLobby){0};
      ...
      LOG_INFO("lobby created: id=%u name=%.31s dynamic=%d", lobby_id,
               lobby->name, (int)is_dynamic);
      return lobby;
    }
  }
  return NULL;
...
static void HandleLobbyListQuery(ENetPeer *peer, const ServerSession *session,
                                 ShroomLobby *lobbies, const ENetHost *host) {
  if (!session->handshake_received) {
    return;
  }
...
static void HandleLobbyJoin(ENetHost *host, ENetPeer *peer,
                            ServerSession *session, ShroomLobby *lobbies,
                            const ENetPacket *enet_packet,
                            uint32_t *next_player_id) {
  const ShroomLobbyJoinPacket *packet =
      (const ShroomLobbyJoinPacket *)enet_packet->data;
  ShroomLobby *lobby;

  if (!session->handshake_received ||
      (enet_packet->dataLength < sizeof(*packet))) {
    return;
  }
  ...
  if (!session->spectating) {
    session->player =
        ShroomWorldSpawnPlayer(&lobby->world, (*next_player_id)++, false);
    if (session->player == NULL) {
      session->lobby_id = 0;
      return;
    }
  ...
  LOG_INFO("lobby join: player_id=%u lobby_id=%u spectating=%d",
           session->player_id, lobby->lobby_id, (int)session->spectating);
...
static void HandleLobbyLeave(ENetHost *host, ServerSession *session) {
  ...
  LOG_INFO("lobby leave: player_id=%u lobby_id=%u", session->player_id,
           session->lobby_id);
  session->lobby_id = 0;
...
static void HandleLobbyCreate(ENetPeer *peer, const ServerSession *session,
                              ShroomLobby *lobbies,
                              const ENetPacket *enet_packet,
                              uint32_t *next_player_id,
                              uint32_t *next_lobby_id) {
  const ShroomLobbyCreatePacket *packet =
      (const ShroomLobbyCreatePacket *)enet_packet->data;
  const ShroomLobby *lobby;
...
static void DispatchInputPacket(ServerPacketContext *context) {
  ShroomLobby *input_lobby =
      FindLobbyById(context->lobbies, context->session->lobby_id);
...
static void DispatchAuthRequestPacket(ServerPacketContext *context) {
  HandleAuthRequestPacket(context->peer, context->session, context->auth_ctx,
                          context->enet_packet);
}

static void DispatchPingPacket(ServerPacketContext *context) {
  SendPong(context->peer,
           ((const ShroomPingPacket *)context->enet_packet->data)->nonce);
}

static void DispatchChatPacket(ServerPacketContext *context) {
  HandleChatPacket(context->host, context->session, context->enet_packet,
                   context->now_ms);
}

static void DispatchLobbyListQuery(ServerPacketContext *context) {
  HandleLobbyListQuery(context->peer, context->session, context->lobbies,
                       context->host);
}

static void DispatchLobbyJoin(ServerPacketContext *context) {
  HandleLobbyJoin(context->host, context->peer, context->session,
                  context->lobbies, context->enet_packet,
...
static void DispatchLobbyCreate(ServerPacketContext *context) {
  HandleLobbyCreate(context->peer, context->session, context->lobbies,
                    context->enet_packet, context->next_player_id,
                    context->next_lobby_id);
}
...
static const ServerPacketDispatchEntry *
FindServerPacketDispatchEntry(ShroomPacketType type) {
  for (size_t index = 0;
       index < sizeof(kServerPacketDispatch) / sizeof(kServerPacketDispatch[0]);
       ++index) {
...
static void HandlePacket(ENetHost *host, ENetPeer *peer, ServerSession *session,
                         ...
                         uint32_t *next_player_id, uint32_t *next_lobby_id,
                         uint64_t now_ms) {
  ...
  if ((enet_packet == 0) ||
      (enet_packet->dataLength < sizeof(ShroomPacketHeader))) {
    return;
  }
...
int main(int argc, char **argv) {
  const uint64_t tick_interval_nanos =
      1000000000ull / (uint64_t)SHROOM_SERVER_TICK_RATE;
  const uint64_t snapshot_interval_ticks =
      (uint64_t)(SHROOM_SERVER_TICK_RATE / (float)SHROOM_SNAPSHOT_RATE);
  const uint64_t spore_interval_ticks =
      (uint64_t)(SHROOM_SERVER_TICK_RATE / (float)SHROOM_SPORE_STATE_RATE);
  ...
  host = enet_host_create(&address, SHROOM_SERVER_MAX_CLIENTS,
                          SHROOM_ENET_CHANNEL_COUNT, 0, 0);
  if (host == 0) {
  ...
  LOG_INFO("shroomio server listening on %s:%u/udp", config.bind_host,
           config.port);
  ...
  while (ShroomLifecycleIsRunning(&g_lifecycle) &&
    ...
    while (enet_host_service(host, &event, 0) > 0) {
      ...
      case ENET_EVENT_TYPE_CONNECT:
        if (event.peer->incomingPeerID < SHROOM_SERVER_MAX_CLIENTS) {
          ...
          LOG_INFO("peer connected: slot=%u",
      ...
      case ENET_EVENT_TYPE_RECEIVE:
        HandlePacket(host, event.peer, (ServerSession *)event.peer->data,
                     lobbies, &auth_ctx, event.packet, event.channelID,
                     &next_player_id, &next_lobby_id, now_ms);
      ...
      case ENET_EVENT_TYPE_DISCONNECT: {
        ...
        LOG_INFO("peer disconnected: slot=%u",
                 (unsigned)event.peer->incomingPeerID);
    ...
    /* Tick and broadcast per-lobby. */
    {
      ...
      for (li = 0; li < SHROOM_MAX_LOBBIES; ++li) {
        ...
        /* Keep empty lobbies discoverable, but do not spend tick budget
         * simulating offscreen bots until someone is actually watching or
         * playing there. */
        if ((real_count + spec_count) == 0u) {
          if (lobby->is_dynamic) {
            if (lobby->empty_since_ms == 0) {
              ...
              LOG_INFO("lobby expired: id=%u name=%.31s", lobby->lobby_id,
                       lobby->name);
              lobby->active = false;
            }
          }
        ...
        /* Roster players are present in snapshots but have not entered gameplay
         * yet. */
        for (pi = 0; pi < host->peerCount; ++pi) {
          ServerSession *session = (ServerSession *)host->peers[pi].data;

          if ((host->peers[pi].state == ENET_PEER_STATE_CONNECTED) &&
              (session != NULL) && session->active && !session->spectating &&
              !session->entered_match &&
              (session->lobby_id == lobby->lobby_id) &&
              (session->player != NULL) && session->player->alive) {
            session->player->spawn_protection_timer =
                SHROOM_PLAYER_SPAWN_PROTECTION_SECONDS;
          }
        }
        ...
        /* Reset ai_controlled for sessions whose focused piece merged or was
         * consumed. */
        {
          size_t ai;
          for (ai = 0; ai < host->peerCount; ++ai) {
            ServerSession *s = (ServerSession *)host->peers[ai].data;
            if ((host->peers[ai].state == ENET_PEER_STATE_CONNECTED) &&
                (s != NULL) && s->active && (s->lobby_id == lobby->lobby_id)) {
        ...
        if ((snapshot_interval_ticks > 0) &&
            ((lobby->world.tick % snapshot_interval_ticks) == 0)) {
          for (pi = 0; pi < host->peerCount; ++pi) {
        ...
        if ((spore_interval_ticks > 0) &&
            ((lobby->world.tick % spore_interval_ticks) == 0)) {
          for (pi = 0; pi < host->peerCount; ++pi) {
            const ServerSession *s =
                (const ServerSession *)host->peers[pi].data;
    ...
    if ((now_ms - last_health_log_ms) >= 60000ull) {
      ...
      for (li = 0; li < SHROOM_MAX_LOBBIES; ++li) {
        if (lobbies[li].active) {
          ...
          LOG_INFO("health lobby_id=%u real=%u bots=%u spectators=%u",
                   lobbies[li].lobby_id, real, bots, spec);
        }
      }
    }
