...
void GameDraw(const Game *game) {
    ...
    DrawText(TextFormat("Mass: %.0f  Radius: %.1f  Speed: %.0f", game->local_player->mass, game->local_player->radius, ShroomMassToSpeed(game->local_player->mass)), 40, 110, 20, GRAY);
    DrawText(TextFormat("Zone: %s  Players: %d  Spores: %d  Tick: %llu", GetZoneLabel(zone), (int)game->world.player_count, (int)game->world.spore_count, game->world.tick), 40, 136, 20, GRAY);
    DrawText(TextFormat("Server: %s  Snapshot Tick: %llu", ClientNetStatusLabel(&game->net), game->net.last_snapshot_tick), 40, 162, 20, game->net.status == CLIENT_NET_CONNECTED ? GREEN : ORANGE);
    ...
    for (index = 0; index < shown_count; ++index) {
        ...
        DrawText(TextFormat("%d. %s %u   %.0f", (int)(index + 1), label, player->player_id, player->mass), 1062, 72 + ((int)index * 22), 20, player == game->local_player ? RAYWHITE : GRAY);
    }
