...
static void SendHello(ClientNetState *net) {
    ...
    enet_peer_send(net->peer, SHROOM_ENET_CHANNEL_CONTROL, CreatePacket(&packet, sizeof(packet), ENET_PACKET_FLAG_RELIABLE));
}

static void SendInput(ClientNetState *net, ShroomVec2 input_direction) {
    ShroomInputPacket packet = {0};

    if (!net->welcome_received || (net->peer == 0) || (net->peer->state != ENET_PEER_STATE_CONNECTED)) {
        return;
    }
...
static void HandleSnapshot(ClientNetState *net, const ENetPacket *enet_packet) {
    ...
    if (player_count > 0) {
        memcpy(net->snapshot_players, packet->players, (size_t)player_count * sizeof(packet->players[0]));
    }
}
